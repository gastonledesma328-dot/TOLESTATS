diff --git a/tole/src/app/api/partidos/route.ts b/tole/src/app/api/partidos/route.ts
index 2730d64e249e821cb44d3156ebebe99a414a2c8d..2d2f0cf3183b312e0ddfdefdf7c67da92122722d 100644
--- a/tole/src/app/api/partidos/route.ts
+++ b/tole/src/app/api/partidos/route.ts
@@ -1,378 +1,456 @@
...
-  const partidos: any[] = [];
+async function fetchPartidosESPN(fecha: string): Promise<Partido[]> {
...
-                    let status = 'Próximo';
-                    let isLive = false;
+        const eventGroups = configData.page?.content?.events as unknown;
+        if (Array.isArray(eventGroups)) {
+          console.log(`Found ${eventGroups.length} event groups in config`);
+
+          eventGroups.forEach((groupItem) => {
+            if (Array.isArray(groupItem)) {
+              const eventGroup: EspnEventGroup = groupItem;
+              eventGroup.forEach((event: EspnEvent) => {
+                if (event && Array.isArray(event.competitors)) {
+                  const homeTeam = event.competitors.find((c) => c.isHome === true);
+                  const awayTeam = event.competitors.find((c) => c.isHome === false);
+
+                  if (homeTeam && awayTeam) {
+                    // Determinar el estado del partido
+                    let status = 'Próximo';
...
+}
+
+function parseHTMLTraditional(html: string): Partido[] {
+  const partidos: Partido[] = [];
+  let matchId = 100; // Empezar desde 100 para diferenciarlo de JSON
+
+  try {
+    // Buscar patrones de equipos con logos en el HTML
+    const teamPattern = /<a[^>]*data-track-nav_item=":schedule:team"[^>]*href="[^"]*">(?:<img[^>]*src="([^"]*)"[^>]*>)?([^<]+)<\/a>/g;
+    const teamMatches: { name: string; logo: string | null }[] = [];
+    let match;
+
+    while ((match = teamPattern.exec(html)) !== null) {
+      teamMatches.push({
...
+function generateFallbackData(): Partido[] {
+  return [
+    {
+      id: 1,
+      homeTeam: 'Boca Juniors',
